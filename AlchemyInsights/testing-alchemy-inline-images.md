---
title: Alchemy インライン画像のテスト
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: ea91a0ef0e003d1a2444d4d5e72ccbdd608bf102
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 08/22/2019
ms.locfileid: "36507168"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="79c09-102">[Domains] ページにアクセスする</span><span class="sxs-lookup"><span data-stu-id="79c09-102">Access the Domains Page</span></span>

<span data-ttu-id="79c09-103">[Domains] ページにアクセスするには、下記のこのアイコンをクリックします。</span><span class="sxs-lookup"><span data-stu-id="79c09-103">To access the domains page, just click this icon:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="79c09-104">ドメインの追加、変更、または削除を行うことができます。</span><span class="sxs-lookup"><span data-stu-id="79c09-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="79c09-105">詳細については、「[ドメインに関する FAQ](https://support.office.com/article/1272bad0-4bd4-4796-8005-67d6fb3afc5a.aspx)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="79c09-105">For more information, see the [Domains FAQ.](https://support.office.com/article/1272bad0-4bd4-4796-8005-67d6fb3afc5a.aspx)</span></span>
  
---
title: 企業向け Office 365 の試用版を拡張します。
ms.author: cmcatee
author: cmcatee-MSFT
manager: mnirkhe
ms.date: 3/20/2018
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Priority
ms.collection: Adm_O365
ms.custom: Adm_O365
ms.assetid: c3fffed1-33b2-4382-ae99-c3a4816497e6
ms.openlocfilehash: c325ca115037831d220d9dd5a7b092ab74aee593
ms.sourcegitcommit: d6ea5e9458a2b8ceaab3ac4bd483e1130b9a398a
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/15/2019
ms.locfileid: "28297101"
---
# <a name="extend-your-trial-for-office-365-for-business"></a>企業向け Office 365 の試用版を拡張します。

試用版サブスクリプションの期限が切れる 15 日以内で、試用版をする前に拡張されていない場合は、別の 30 日間の試用版を拡張できます。この 1 回のみ行うことができます。
  
1. 管理センターで、[[サブスクリプション](https://go.microsoft.com/fwlink/p/?linkid=842054)] ページに移動または**請求先**を選択\>**のサブスクリプション**。
    
2. 拡張するための試用版サブスクリプションを選択します。
    
3. **試用版の拡張**を選択し、ウィザードの手順を完了します。 
    
4. 試用版を拡張するためのクレジット カード情報を提供する必要があります。試用版の拡張機能のクレジット カードを充電できません。
    
試用版を拡張する方法や手順の詳細については、[拡張するビジネス向けの Office 365 の試用版](https://support.office.com/article/75533195-f1f6-4c2c-8ceb-0b5597790d7b)を参照してください。
  


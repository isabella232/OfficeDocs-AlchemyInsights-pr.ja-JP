---
title: 8000059 優先受信トレイを無効にする
ms.author: daeite
author: daeite
manager: jackiesm
ms.date: 9/17/2018
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 59d6cc6c-67ca-41df-baaf-2a6f94531439
ms.openlocfilehash: 7bdd7a9cff33cdeef9f9e11c94653046007916fd
ms.sourcegitcommit: dd43cc0a9470f98b8ef2a3787c823801d674c666
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 02/12/2019
ms.locfileid: "29905801"
---
# <a name="turn-off-focused-inbox"></a><span data-ttu-id="1f51e-102">優先受信トレイを無効にする</span><span class="sxs-lookup"><span data-stu-id="1f51e-102">Turn off Focused Inbox</span></span>

<span data-ttu-id="1f51e-p101">優先受信トレイを使用すると、最も重要なメールを優先的に扱うことができます。読む可能性の高いメールは [優先] 下に表示され、残りのメールは [その他] に移動されます。その仕組みについては、[こちらをご覧ください](https://support.office.com/article/f445ad7f-02f4-4294-a82e-71d8964e3978#bkmk_consumer)。優先受信トレイを使用しない場合は、下のボタンを選択します。</span><span class="sxs-lookup"><span data-stu-id="1f51e-p101">Focused Inbox helps you focus on what matters most. Email you're likely to read appears under Focused and the rest is moved to Other. To learn how it works, [read this](https://support.office.com/article/f445ad7f-02f4-4294-a82e-71d8964e3978#bkmk_consumer). If you don't want to use Focused Inbox, select the button below.</span></span>
  


...
SharePoint または OneDrive では、次のような理由で、処理が遅くなるか、アクセスできないか、またはサービスが利用できない、または503のエラーが表示されることがあります。
